...
> Note: Telegram relay and the pi extension now live in separate repos (`telegram-relay`, `pi-extension-backbone-client`).
...
- `REMINDER_WEBHOOK_URL` (enable webhook dispatch)
- `REMINDER_WEBHOOK_SECRET` (HMAC secret for webhook)
...
- `userExternalId` is required on all `/api/*` requests (no default fallback)
